...
sf::Vector2i TableElement::cell_size = {500, 40 };
sf::Vector2i TableElement::header_size = { 90, 40 };
...
void TableElement::draw(sf::RenderTarget &target, sf::RenderStates states) const {
    ...
    {
        RoundedRectangleShape frame{sf::Vector2f(real_size.x - 4, real_size.y - 4)};
    ...
    // net + vals
    {
        ...
        net_texture.display();
        net_sprite.setPosition(12 + header_size.x, cell_size.y + 12);
...

        {
            ...
            for (int i = -pos_in_.x / cell_size.x; i * cell_size.x + pos_in_.x < real_size.x && i < machine_.Size().x; ++i) {
                for (int j = -pos_in_.y / cell_size.y; j * cell_size.y + pos_in_.y < real_size.y && j < machine_.Size().y; ++j) {
                    ...
                    str.setPosition(i * cell_size.x + 5 + pos_in_.x,
        ...
        net_sprite.setPosition(header_size.x * 2 + 12 + pos_.x * win_size.x, cell_size.y * 2 + 12 + pos_.y * win_size.y);
    ...
    for (int i = 0; i < machine_.Size().y; ++i) {
        ButtonWithTextAbsPos erase_q(sf::Vector2f(12 + header_size.x * 0.5, (i + 0.5) * cell_size.y + pos_in_.y),
                                     sf::Vector2f(40, 30),
                                     "del",
    ...
    {
        sf::RenderTexture qs;
        qs.create(header_size.x * 2, real_size.y - cell_size.y * 2 - 24);
        ...
        for (int i = -pos_in_.y / cell_size.y; i < machine_.Size().y && i * cell_size.y + pos_in_.y < std::min(real_size.y, machine_.Size().y * cell_size.y); ++i) {
            ...
            str.setPosition(sf::Vector2f(header_size.x * 1.5, i * cell_size.y + cell_size.y / 2 + pos_in_.y));
            qs.draw(str);
            qs.draw(buttons_qs_[i]);
        }
        buttons_qs_mutex_.unlock();
        sf::RectangleShape line(sf::Vector2f(header_size.x * 2, 2));
        line.setFillColor(outline);
    ...
    // syms
    {
        ...
        syms.create(real_size.x - header_size.x * 2 - 24, cell_size.y * 2);
    ...
    {
        sf::RectangleShape legend_border(sf::Vector2f(2, std::min(real_size.y - 24, machine_.Size().y * cell_size.y + cell_size.y * 2)));
        legend_border.setFillColor(outline);
        legend_border.setPosition(sf::Vector2f(pos_.x * win_size.x + 12 + header_size.x * 2, pos_.y * win_size.y + 12));
        target.draw(legend_border);
    }
    ...
    // frame
    {
        ...
        table_border.setPosition(sf::Vector2f(pos_.x * win_size.x + 12, pos_.y * win_size.y + 12));
...
sf::Vector2i TableElement::GetTableSize() const {
    return { cell_size.x * machine_.Size().x + header_size.x * 2 / 2,cell_size.y * (machine_.Size().y + 1) };
}
